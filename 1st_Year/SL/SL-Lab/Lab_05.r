...
# argument se.fit=TRUE for standard errors
#
...
# this is the default value for objects 
# returned by glm()

anova(output.R, output.F)

# you get an error message if you apply the
# F test in this case

anova(output.R, output.F, test="F")
...
output.F <- glm(default ~ balance+student+income, family = binomial)
summary(output.F)
output.R <- glm(default ~ balance, family = binomial)
anova(output.R, output.F, test="Chisq")
